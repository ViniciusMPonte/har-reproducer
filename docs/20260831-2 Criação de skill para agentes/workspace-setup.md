...
```
<raiz>/<domínio>__<AAAAMMDD>/
  ├── <arquivo>.har        # cópia do HAR original
  └── output/              # workspace de saída (--output aponta aqui)
```

- **Nome da pasta**: `<domínio>__<AAAAMMDD>`, mesma convenção já usada em
  `tests/real/captures/` no próprio projeto — `<domínio>` é o domínio do
  portal alvo, `<AAAAMMDD>` é a data da **captura** do HAR (quando o fluxo
  foi gravado), não a data em que o agente está trabalhando nele.
...
Cada HAR novo recebe sua própria pasta `<domínio>__<AAAAMMDD>/` — o agente
não reaproveita nem combina o workspace de um HAR dentro do de outro, mesmo
que sejam do mesmo domínio. Se dois HARs do mesmo domínio forem capturados
em datas diferentes, ambos coexistem lado a lado (`exemplo.com.br__20260824/`,
`exemplo.com.br__20261102/`), do mesmo jeito que já acontece em
`tests/real/captures/`.
